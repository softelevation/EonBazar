    ...
    //    center
      ...
      >
        ...
        <Text
          left
          margin={[hp(0.5), 0, 0, 0]}
          style={{ left:60, color: 'black', fontSize: 15,top:-30 }}
          height={16}
          transform="uppercase"
          size={10}>
          {item.name}
        </Text>
        {
          item.children_data.map((element, index) => {
            return <TouchableOpacity onPress={() => onPress(element)} style={{ flex: 1,with:'100%', height: 50,margin:10}}>
              <Text style={{ color: 'black', fontSize: 12, textAlign: 'left', left:60,top:-30 }}> {element.name} </Text>
            </TouchableOpacity>
          })
        }
 <View style={{borderWidth:1,borderColor:'#d3d3d3'}}></View>
